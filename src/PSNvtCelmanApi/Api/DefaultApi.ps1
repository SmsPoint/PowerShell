...
function Invoke-SmsSend {
    ...
    Process {
        'Calling method: Invoke-SmsSend' | Write-Debug
        ...
        if (!$SmsSendRequest) {
            throw "Error! The required parameter `SmsSendRequest` missing when calling smsSend."
        }

        $LocalVarBodyParameter = $SmsSendRequest | ConvertTo-Json -Depth 100
